...
pub trait JsonWriter<K, V> {
    ...
    fn json_objectstart<V>(&mut self, value: &V) -> &mut Self;
    fn json_arraystart(&mut self, value: &V) -> &mut Self;
    fn json_objectend(&mut self) -> &mut Self;
    fn json_arrayend(&mut self) -> &mut Self;
    fn json_keyvalue(&mut self, key: &K, value: &V) -> &mut Self;
}
...
impl<K: Display, V: Display> JsonWriter<K, V> for Context {
    fn indent(&mut self) -> &mut Self {
        self.indent += 2;
        self
    }
    ...
    fn json_objectstart(&mut self, value: &V) -> &mut Self {
        match self.state {
            JSONState::AfterValue => {
                self.write(",");
            }
    ...
    fn json_keyvalue(&mut self, key: &K, value: &V) -> &mut Self {
        if self.state = JSONState::AfterValue {
            self.write(",");
        }
        self.write_new_line()
            .advance()
...
impl Context {
    ...
    fn write(&mut self, string: impl Display) -> &mut Self {
        self.output.push_str(format!("{}", string).as_str());
        self
    }
}
